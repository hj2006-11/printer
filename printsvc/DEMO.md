...
2. 自动轮询（1.5s/次），约 9~14 秒后（本机实测多轮 12~14s）状态变为 `"completed"`，响应含 `output_path`。

> 耗时说明：渲染约 2s；本机无实体打印机时，打印阶段因「另存为」对话框在无交互会话中**快速失败**（D7 实测，非 10s 挂起），随即走 PDF 兜底（视为成功），故总计约 9~14s。期间查询为 `pending` 属正常异步时序，不是卡死。
...
| Linux 真机验证 | 部分完成（D6 G1） | 交叉编译通过（ELF64），运行验证待演示前借真机 |
