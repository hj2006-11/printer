...
> 临时假实现说明：当前版本未接实体打印机验证出纸（D7 待办），无打印机时打印模块自动回退为 PDF 兜底并视作成功——这是**有意的临时行为**，非 bug。注意无打印机时打印阶段会阻塞至 10s 超时后才兜底，故任务从提交到 `completed` 约需 12s（详见 S5）。气球小票/代码渲染均为真实实现。
...
| Linux 真机验证 | 部分完成（D6 G1） | 交叉编译通过（ELF64），运行验证待 D7 借真机 |
