...
> 临时假实现说明：当前版本未接实体打印机验证出纸（D8 待办），无打印机时打印模块自动回退为 PDF 兜底并视作成功——这是**有意的临时行为**，非 bug。注意无打印机时打印阶段会失败后走 PDF 兜底，任务从提交到 `completed` 约需 9~14s（D7 实测，详见 S5）。D7 起可用环境变量 `CCPC_PRINTER` 指定打印机（Windows 用 printui 切换默认后打印并恢复，Linux 用 `lp -d`），未设置时使用系统默认打印机。气球小票/代码渲染均为真实实现。
...
| 项 | 状态 | 说明 |
|---|---|---|
| 实体打印机出纸 | 部分完成（D7 G4） | `CCPC_PRINTER` 指定打印机已实现并实测真实打印链（虚拟打印机因保存对话框无交互→PDF 兜底）；实体出纸待 D8 借实机 |
| 指定打印机 | 已实现（D7 G4） | 环境变量 `CCPC_PRINTER=<打印机名>`；Windows printui 切换默认→打印→恢复，Linux `lp -d`；未设置行为不变 |
